...
export async function GET({ locals, request }: { locals: App.Locals; request: Request }) {
  ...
  const fixedPaths = ['/', '/learn/', '/life/', '/life/essays/', '/life/anime/', '/life/gallery/', '/archive/', '/about/', '/search/'];
  const urls = [...fixedPaths.map((path) => ({ path, updated: null })), ...documents.map((entry) => ({ path: documentHref(entry), updated: entry.updated_at }))];
